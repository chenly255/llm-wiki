...
| 环节 | 做什么 | 你需要做的 |
|------|--------|-----------|
| ① 多模态摄入 | 论文/文章/代码/网页/推文/视频 全丢进 inbox | 丢文件或贴链接 |
| ② 智能编译 | AI 提炼概念、实体、跨文档关联，生成知识图谱 | 一键触发 |
...
不只是本地文件——**网页、推文、论文链接**都能直接丢进 inbox，一行命令批量消化：

| 来源 | 提取方式 | 状态 |
|------|---------|------|
| PDF / 本地文件 | Read 工具直接提取（零配置）；可选 pymupdf4llm / [MinerU](https://github.com/opendatalab/MinerU) 高保真转换 | ✅ 已支持 |
| 代码 / 笔记 (PY, IPYNB) | 直接读取 | ✅ 已支持 |
| 纯文本粘贴 | 直接使用 | ✅ 已支持 |
| 网页 / 博客 / Medium | web_reader 自动提取 Markdown | ✅ 已支持 |
| X / Twitter | web_reader 自动提取 | ✅ 已支持 |
| GitHub / GitLab 仓库 | GitHub MCP 提取目录树 + README + 关键代码，LLM 生成结构化概览 | ✅ 已支持 |
| YouTube | `/youtube-render-pdf` skill 生成结构化笔记 + PDF | ✅ 已支持（需安装 skill） |
| Bilibili | `/bilibili-render-pdf` skill 生成结构化笔记 + PDF | ✅ 已支持（需安装 skill） |
| 微信公众号 | tavily_extract 提取（需生产 key） | 🔧 待验证 |
| 知乎 | 反爬拦截，建议手动复制 | 🔧 待接入 |
| 小红书 | 需安装 [xiaohongshu-mcp](https://github.com/xpzouying/xiaohongshu-mcp)（免费，需 Cookie） | 🔧 可接入 |

把 URL 列表存成 `.txt` 文件扔进 inbox，digest 自动批量处理。大部分场景不需要装任何额外工具——利用 Claude Code 内置的 MCP 工具链搞定。
